...
class GUI(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("3rd YEAR PROJECT CHECKERS/DRAUGHTS")
        # draws a black piece
    ...
    def resign(self):
        ...
        if self.playing:
            resignBox = QMessageBox.question(self,'Resigning','Are you sure you want resign and forfeit?',QMessageBox.Yes,QMessageBox.No)
        ...
        if not self.game.isOver():
            ...
            if self.x1 == None : 
                ...
                if(self.game.getTurn() == "Player"):
                    moved = self.game.movePiece("Player",self.x1, self.y1, self.x2,self.y2)
                # updates the board
                self.updateGame()    
    ...
    def closeEvent(self,event):
        reply = QMessageBox.question(self,'Message','Are you sure you want to close the program?',QMessageBox.No,QMessageBox.Yes)
